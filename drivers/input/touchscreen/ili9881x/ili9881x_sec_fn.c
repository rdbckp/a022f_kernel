		...
		const char *buf, size_t count)
{
	...
	case LCD_OFF:
        if (buff[1] == LCD_EARLY_EVENT) {
             /* Paksa panggil RESUME/ON meskipun sistem minta OFF */
             /* Biar digitizer tetep nyala buat nangkep double tap */
             if (ili_sleep_handler(TP_EARLY_RESUME) < 0) 
                     input_err(true, ilits->dev, "GSI Hack: Keep TP Awake failed\n");
        }
        break;
	case LCD_ON:
		if (buff[1] == LCD_EARLY_EVENT) {
