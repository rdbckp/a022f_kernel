...
int ili_sleep_handler(int mode)
{
	...
	case TP_EARLY_SUSPEND:
		...
		if (1) { // PAKSA AKTIF! Biar masuk mode gesture terus pas layar mati
	...
	case TP_RESUME:
		...
		/* GSI HACK: Kalo udah siaga dari mode gesture, 
           cukup pastiin pin nyala, gak usah reset-reset lagi */
		ilitek_pin_control(true);

		// Kita "bypass" ritual reset yang bikin lama
		if (ilits->tp_suspend) {
			input_info(true, ilits->dev, "GSI Hack: Fast Resume Active!\n");
			ili_ic_func_ctrl_reset(); // Cukup reset fungsi, bukan reset hardware
		}
...
int ili_fw_upgrade_handler(void *data)
{
	...
	/* GSI HACK: Kunci Utama dari Dev Pro */
	/* Kalau statusnya bukan 'force upgrade', jangan ganggu proses resume */
	if (ilits->boot && ilits->fw_update_stat != FW_STAT_FORCED) {
		input_info(true, ilits->dev, "GSI Hack: Resume detected, bypassing heavy firmware check\n");
		goto skip_heavy_stuff;
	}

	atomic_set(&ilits->fw_stat, START);
	ilits->fw_update_stat = FW_STAT_INIT;
	
	/* Biang kerok delay kita matikan di sini */
	...
	if (ret != 0) {
		...
		input_info(true, ilits->dev, "GSI Hack: FW check skipped\n");
		ilits->fw_update_stat = FW_UPDATE_PASS;
	}

skip_heavy_stuff:

	/* Bagian ini HARUS JALAN supaya GAK BOOTLOOP */
	if (!ilits->boot) {
		ilits->boot = true;
		input_info(true, ilits->dev, "GSI Hack: Initializing input device\n");
		ili_input_register();
		ili_wq_ctrl(WQ_ESD, ENABLE);
		ili_wq_ctrl(WQ_BAT, ENABLE);
	}
...
int ili_report_handler(void)
{
	...
	if (ilits->actual_tp_mode == P5_X_FW_GESTURE_MODE) {
        ...
        if (ilits->pm_suspend) {
            /* GSI HACK START: Tendangan Maut DT2W */
            input_info(true, ilits->dev, "GSI Hack: Gesture Detected! Waking up GSI NOW!\n");
            
            // 1. Kirim sinyal bangun ke kernel
            pm_wakeup_event(ilits->dev, 0);
            
            // 2. Simulasi pencet tombol Power (KEY_WAKEUP)
            // Ini yang bikin GSI lu langsung nyalain layar tanpa nunggu resume
            input_report_key(ilits->input_dev, KEY_WAKEUP, 1);
            input_sync(ilits->input_dev);
            input_report_key(ilits->input_dev, KEY_WAKEUP, 0);
            input_sync(ilits->input_dev);
            
            /* Biarkan sistem lanjut ke proses resume asli supaya nggak crash */
            /* GSI HACK END */
