...
mkdir toolchain2
cd toolchain2
if [ ! -d clang ]; then mkdir clang && curl -Lsq "${{ inputs.CLANG_AOSP }}" -o clang.tgz && tar -xzf clang.tgz -C clang; fi
if [ ! -d gcc ]; then mkdir gcc && curl -Lsq "${{ inputs.GCC_AOSP }}" -o gcc.tgz && tar -xzf gcc.tgz -C gcc; fi
cd ${GITHUB_WORKSPACE}
